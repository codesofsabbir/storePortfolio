      ...
      {data.map((dt, index) => {
        ...
        return (
          <Link
            key={index}
            href={`/admin/${dt.link}`}
            className={`group relative flex items-center gap-3 h-12 rounded-xl overflow-hidden ${
                isActive
          ...
          >
            ...
            <i className="w-12 h-12 flex justify-center items-center">{dt.icon}</i>
            <span className={`transition-all duration-700 ease-in-out ${isSidebarOpen? "opacity-100 ml-0" : "opacity-0 ml-5"}`}>{dt.title}</span>
            </div>
            {/* Tooltip */}
            {isSidebarOpen ? (
              ""
